...
from custom_swin_unetr import (SwinTransformerBlock, PatchMerging, get_window_size, compute_mask)
...
class DualAttnUNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.dualattn_transformer = DualAttnFormer(img_in_chans=img_in_chans, lang_in_chans=lang_in_chans, dim=feature_size, seq_length=seq_length, image_size=img_size, window_size=self.window_size, patch_size=self-patch_size, num_heads=num_heads, depths=depths)

        self.input_encoder = UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=img_in_chans, out_channels=feature_size, kernel_size=3, stride=1, norm_name=norm_name, res_block=True,)

        self.encoder_list = nn.ModuleList()

        for stage in range(len(depths)):
            channels = feature_size if stage == 0 else feature_size*2**(2**(stage-1))
            encoder = UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=channels, out_channels=channels, kernel_size=3, stride=1, norm_name=norm_name, res_block=True,)
            self. encoder_list. append (encoder)

        self.decoder_list = nn.ModuleList()

        for stage in range(len(depths)+1):
            decoder = UnetrUpBlock(spatial_dims=spatial_dims, in_channels=feature_size*2**(len(depths) -stage), out_channels=feature_size*2**max(len(depths) -stage-1, 0), kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=True)
            self.decoder_list. append (decoder)

        self.out = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size, out_channels=out_channels)
    

    def forward (self, img_in: torch.FloatTenser, lang_in: torch.FloatTensor) :
...
class DualAttnFormer (torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.patch_embed = PatchEmbed(patch_size=patch_size, in_chans=img_in_chans, embed_dim=dim, norm_layer=norm_layer, spatial_dims=spatial_dims)
    ...
    def _create_block(self, 
        ...
        self.no_shift = tuple(0 for 1 in self.window_size)
        ...
        for i in range(depth):
            ...
            image_query_branch = ImageQueryBranch(c_i=c_i, c_t=c_t, num_heads=num_heads)

            language_query_branch = LanguageQueryBranch(c_i=c_i, c_t=c_t, image_size=image_size, seq_length=seq_length, num_heads=num_heads)

            layer_dict = torch.nn.ModuleDict({f"swin_transformer_{i}": swin_transformer, 
                                              f"image_query_branch_{i}": image_query_branch, 
                                              f"language_query _branch_{i}": language_query_branch})
...
class ImageQueryBranch(torch.nn.Module):
    ...
    def _init_(self, c_i, c_t, num_heads=1) -> None:
        ...
        self.f_query = nn. Sequential(nn.Conv3d (c_i, c_i, kernel_size=1), nn. InstanceNorm3d(c_i))
        ...
        self.proj_w = nn.Sequential(nn.Conv3d (c_i, c_i, kernel_size=1), nn.InstanceNorm3d(c_i))

        # skip projection
        self.proj_img = nn.Sequential(nn.Conv3d(c_i, c_i, kernel_size=1), nn.InstanceNorm3d(c_i))  # nn.ReLU()

        # output projection
        self.proj_out = nn.Sequential(nn.Conv3d (c_i, c_i, kernel_size=1), nn.ReLU())
        
        # gating to balance the language info
        self.gate = nn.Sequential(nn.Conv3d(c_i, c_i, kernel_size=1), nn.ReLU(), nn.Conv3d (c_i, c_i, kernel_size=1), nn.Tanh())


    def forward (self, img_in: torch.Tensor, lang_in: torch.Tenser, lang_mask: torch.Tensor = None):
        ...
        # (b, num_heads, H*W, self.c_i//self.num_heads)
        ...
        sim_map = torch.matmul (query, key)  # (b, num_heads, H*W* (D)//num_heads, 1)
        sim_map = (self.c_i** -.5) * sim_map  # scaled dot product
        sim_map = sim_map + (1e4*lang_mask - 1e4)  # assign a very small number to padding positions
        sim_map = F.softmax(sim_map, dim=-1)  # (b, num_heads, H*W* (D)//num_heads, 1)
...
class LanguageQueryBranch (torch.nn.Module) :
    ...
    def __init__(self, c_i:int, c_t: int, image_size: Sequence[int], seq_length: int, num_heads: int=1) -> None:
        ...
        self.f_key = nn.Sequential(nn.Conv3d(c_i, c_i, kernel_size=1), nn.InstanceNorm3d(c_i))
        self.f_value = nn.Sequential(nn.Conv3d(c_i, c_i, kernel_size=1), nn.InstanceNorm3d(c_i))
    ...
    def forward (self, img_in: torch.Tensor, lang_in: torch.Tensor, lang_mask: torch.Tensor = None):
        ...
        # cross attention
        query = self.f_query(lang_in) * lang_mask  # (b, d_fmap, 1)
        key = self.f_key(img_in)  # (b, c_i, fmap_d, fmap_h, fmap_w)
        value = self.f_value(img_in)  # (b, c_i, fmap_d, fmap_h, fmap_w)
        fd, fw, fh = key.size()[-3:]

        query = query.reshape(b, self.num_heads, self.d_fmap // self.num_heads, 1)  # (b, num_heads, d_fmap// num_heads, 1)
        ...
        sim_map = torch.matmul (query, key)  # (b, num_heads, 1, c_i)
        sim_map = (self.d_fmap ** -.5) * sim_map  # scaled dot product
        sim_map = sim_map + (1e4*lang_mask - 1e4) # assign a very small number to padding positions
        sim_map = F.softmax(sim_map, dim=-1)  # (b, num_heads, 1, c_1)
        x = torch.matmul(sim_map, value.permute(0, 1, 3, 2))  # (b, num_heads, 1, d_fmap//num_heads)
        x = x. permute(0, 2, 1, 3).contiguous().reshape(b, l, self.d_fmap).transpose(-2, -1)  # (B, d_fmap, 1)

        # fusion
        weight = self.proj_w(x)  # (B, d_fmap, 1)
        skip = self.proj_lang(lang_in)  # (B, d_fmap, 1)
        out = self.relu(skip * weight)

        # gating
        gate = self.gate(out)  # (B, d_fmap, 1)
